...
KIBANA_NODE_PORT=${KIBANA_NODE_PORT:-31601}
...
expose_node_port() {
  ...
  iptables -t nat -C PREROUTING -p tcp -i "$PUBLIC_INTERFACE" --dport "$public_port" \
    -j DNAT --to-destination "${MINIKUBE_IP}:${node_port}" 2>/dev/null || \
    iptables -t nat -A PREROUTING -p tcp -i "$PUBLIC_INTERFACE" --dport "$public_port" \
      -j DNAT --to-destination "${MINIKUBE_IP}:${node_port}"
  iptables -t nat -C OUTPUT -p tcp -d "$PUBLIC_IP" --dport "$public_port" \
    -j DNAT --to-destination "${MINIKUBE_IP}:${node_port}" 2>/dev/null || \
    iptables -t nat -A OUTPUT -p tcp -d "$PUBLIC_IP" --dport "$public_port" \
      -j DNAT --to-destination "${MINIKUBE_IP}:${node_port}"
  iptables -t nat -C POSTROUTING -p tcp -d "$MINIKUBE_IP" --dport "$node_port" \
    -j MASQUERADE 2>/dev/null || \
    iptables -t nat -A POSTROUTING -p tcp -d "$MINIKUBE_IP" --dport "$node_port" \
      -j MASQUERADE
  iptables -C FORWARD -p tcp -d "$MINIKUBE_IP" --dport "$node_port" \
    -j ACCEPT 2>/dev/null || \
    iptables -I FORWARD 1 -p tcp -d "$MINIKUBE_IP" --dport "$node_port" \
      -j ACCEPT
  iptables -C FORWARD -p tcp -s "$MINIKUBE_IP" --sport "$node_port" \
    -j ACCEPT 2>/dev/null || \
    iptables -I FORWARD 1 -p tcp -s "$MINIKUBE_IP" --sport "$node_port" \
      -j ACCEPT
}

expose_node_port "$FRONTEND_NODE_PORT" "$FRONTEND_NODE_PORT"
expose_node_port "$KIBANA_LOCAL_PORT" "$KIBANA_NODE_PORT"
